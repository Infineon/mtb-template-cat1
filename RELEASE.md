...
### What Changed?
#### v1.12.0
* Removed the default enabling of all PERI MMIO group slave-control (SL_CTL) bus-clock gates and their associated CLK_HF2/CLK_HF3/CLK_HF4 clocks from `SystemInit_Enable_Peri()`/`SystemInit_Enable_Clocks()` in `system_psc3.c` for PSOC&trade; Control C3 devices. This configuration is now generated by the new `perimmio-intrinsic-1.0` personality, which produces peripheral slave-control (SL_CTL) bus-clock gating code for the PERI MMIO block based on actual peripheral/clock usage instead of unconditionally enabling all groups and clocks at startup.
* Added a USER_NSC_SIZE option to configure the NSC (Non-Secure Callable) region size for PSC3 devices from the Makefile.
* Added suppression of non-functional RWX-segment linker warning for GCC 14 compiller.
* Fixed System_Restore_NVIC_Reg() to restore NVIC priority registers before interrupt-enable registers for CYW20829 and PSOC&trade; Control C3 (PSC3) devices.
* Fixed CM7 GCC linker script DTCM LENGTH to use `_size_DTCM` instead of `_base_DTCM` for XMC7100/XMC7200 and TRAVEO&trade; T2G (tviic) devices.
* Enabled ECC checking for ICACHE in the CM33 startup code (startup_cat1b_cm33.c) for CAT1B devices.
#### v1.11.0
